...
@Component({
  selector: 'app-third-party-sarees',
  templateUrl: './third-party-sarees.component.html',
  styleUrl: './third-party-sarees.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ThirdPartySareesComponent implements OnInit {
  ...
  setThirdPartyDetails(id: any) {
    const newArray = this.thirdPartyDetailsList.filter((e) => { return e.third_partyid == id });
  ...
  async getItemchange(i: any): Promise<void> {
    ...
    const filteredItems = this.itemMasterList.filter((e) => e.brandid === brandid);
  ...
  setHsn(i: any) {
    ...
    Control.at(i).get('hsn_number')?.setValue(this.itemMasterList.filter((e) => { return e.itemid == proID })[0].hsn_number);

    this.colculation(i);
  }

  getThirdPartPurchaseList(dated: any) {
    this.tPPSvc.getThirdPartyPurchaseLists(this.companyID, dated).subscribe((res: any) => {
  ...
  getBrandControl(index: number): FormControl {
    const control = (this.thirdPartyForm.get('thirdparty_nested') as FormArray).at(index)?.get('brandid') as FormControl;
    return control;
  }
  ...
  getItemControl(index: number): FormControl {
    const control = (this.thirdPartyForm.get('thirdparty_nested') as FormArray).at(index)?.get('itemid') as FormControl;
    return control;
  }
  ...
  getPriceControl(index: number): FormControl {
    const control = (this.thirdPartyForm.get('thirdparty_nested') as FormArray).at(index)?.get('price') as FormControl;
    return control;
  }
  ...
  getDiscountControl(index: number): FormControl {
    const control = (this.thirdPartyForm.get('thirdparty_nested') as FormArray).at(index)?.get('discount') as FormControl;
    return control;
  }
  ...
  getQtyControl(index: number): FormControl {
    const control = (this.thirdPartyForm.get('thirdparty_nested') as FormArray).at(index)?.get('qty') as FormControl;
    return control;
  }
  ...
  async findBillNo(): Promise<void> {
    this.tPPSvc.getMaxId(this.companyID).subscribe(data => {
      let maxnumber = 0;
      if (data.length > 0) {
        maxnumber = data[0].purchaseid + 1
        if (maxnumber < 10) {
          this.thirdPartyForm.get('bill_no')?.setValue('0' + String(maxnumber));
        }
        else {
  ...
  async findSiCode(): Promise<void> {
    this.tPPSvc.getThirdPartySICode(this.companyID).subscribe(data => {
      ...
      if (data.length > 0) {
        const Control = this.thirdPartyForm.get('thirdparty_nested') as FormArray;
        const newAlredyUpCount = Control.value.filter((eu: any) => { return eu.purchase_n_id > 0 });
        Control.value.forEach((element: any, i: number) => {
  ...
  async findRefCode(): Promise<void> {
    this.tPPSvc.getThirdPartyRefCode(this.companyID, this.thirdPartyForm.value.third_partyid || 0).subscribe(data => {
      let maxnumber = 0;
      if (data.length > 0) {
        const Control = this.thirdPartyForm.get('thirdparty_nested') as FormArray;
        const newAlredyUpCount = Control.value.filter((eu: any) => { return eu.purchase_n_id > 0 })
  ...
  addNesForm() {
    ...
    (this.thirdPartyForm.get('thirdparty_nested') as FormArray).push(newControl);
      ...
      if (res == true) {
        ...
        this.tPPSvc.addNew(value).subscribe(res => {
          if (res.status == 'Saved successfully') {
            ...
            this.notificationSvc.warn("Bill No or Invoice No already exists ! Please save it again");
          }
          else {
  ...
  async getReport(purchaseid: any) {
    const thirdPartyNongstPurchaseReports = await this.tPPSvc
  ...
  async update(item: any) {
    const nestedArray = await this.tPPSvc.getThirdPartyPurchaseNestedLists(item.purchaseid).toPromise();
    const control = <FormArray>this.thirdPartyForm.controls['thirdparty_nested'];
    ...
    if (control.length == 0) {
      ...
      nestedArray?.forEach(async (e, i) => {
        ...
        const newControl = new FormGroup({
          ...
          discount: new FormControl(e.discount, [Validators.required, Validators.pattern(/^\d+(\.\d{1,2})?$/)]),
