...
@Component({
  selector: 'app-multiple-sales-billentry',
  templateUrl: './multiple-sales-billentry.component.html',
  styleUrl: './multiple-sales-billentry.component.scss',
  providers: [DatePipe]
})
export class MultipleSalesBillentryComponent {
  ...
  getSalesBillingList(dated: any) {
    ...
    this.sBNSvc.getSalesNonGSTList(this.companyID, dated).subscribe((res: any) => {
  ...
  async finalCalculation(): Promise<void> {
    ...
    this.salesBillEntryForm.get('total')?.setValue(String(FormTot.toFixed(2)));
  ...
  async update(item: any) {
    ...
    const newArray = await this.sBNSvc.getSalesNonGSTNestedList(item.entryid).toPromise();
