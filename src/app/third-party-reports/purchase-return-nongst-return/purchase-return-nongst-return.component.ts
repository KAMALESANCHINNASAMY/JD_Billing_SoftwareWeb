...
@Component({
  selector: 'app-purchase-return-nongst-return',
  templateUrl: './purchase-return-nongst-return.component.html',
  styleUrl: './purchase-return-nongst-return.component.scss'
})
export class PurchaseReturnNongstReturnComponent implements OnInit {
  thirdPartyDetailsList: any[] = [];
  returnReportsAll: any[] = [];
  returnReports: any[] = [];
  ...
  getThirdPartyPurReportByFromTodate() {
    if (this.reportForm.valid) {
      ...
      this.TMRSVC.getThirdPartyNongstPurchaseReturnFromToDate(fromDate, toDate, cusId, this.companyID).subscribe((res) => {
  ...
  total_nettotal() {
    ...
    return_net_total = this.returnReports.reduce((acc: any, item: any) => (acc += Number(item.return_total)), 0);
